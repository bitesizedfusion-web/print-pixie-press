    ...
    <div>
      <section className="bg-hero-bg text-hero-foreground py-16">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="mt-2 text-hero-foreground/70">{items.length} item{items.length !== 1 ? 's' : ''} in your cart</p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto px-4">
          {items.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
              ...
              <Link to="/products"><Button variant="cta" size="lg" className="mt-6">Browse Products</Button></Link>
            </motion.div>
          ) : (
            <div className="space-y-6">
              {items.map((item, i) => (
                <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                  ...
                  <div className="w-full sm:w-32 h-24 rounded-lg overflow-hidden shrink-0">
                    <img src={item.product.image} alt={item.product.name} className="w-full h-full object-cover" loading="lazy" width={128} height={96} />
                  </div>
                  <div className="flex-1 space-y-1">
                    <h3 className="font-heading font-bold text-foreground">{item.product.name} — {item.size}</h3>
                    <p className="text-sm text-muted-foreground">
                      {item.quantity} × {paperTypes.find(p => p.id === item.paperType)?.name} · {item.doubleSided ? "Double Sided" : "Single Sided"} · {item.express ? "Express" : "Standard"}
                    </p>
                    {item.fileName && <p className="text-xs text-muted-foreground">File: {item.fileName}</p>}
                  </div>
                  <div className="flex sm:flex-col items-center sm:items-end justify-between gap-2">
                    <span className="font-mono text-lg font-bold text-cta">AUD ${item.total.toFixed(2)}</span>
              ...
              <div className="bg-card rounded-xl border border-border p-6">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-mono text-2xl font-bold text-cta">AUD ${cartTotal.toFixed(2)}</span>
                </div>
                <Link to="/checkout">
