  ...
  return (
    <div className="p-8">
            ...
            <DialogContent>
              ...
              <form onSubmit={submit} className="space-y-3">
                ...
                <Button type="submit" variant="cta" className="w-full" disabled={busy}>
                  {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "Log Expense"}
                </Button>
              </form>
            </DialogContent>
          </Dialog>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          <div className="flex items-center gap-2 text-muted-foreground text-xs uppercase tracking-wider">
            <DollarSign className="h-3 w-3" /> This Month
          </div>
          ...
          columns={[
            {
              key: "expense_date",
              label: "Date",
              render: (r) => <span className="text-xs">{r.expense_date}</span>,
            },
            {
              key: "category",
              label: "Category",
            ...
            },
            {
              key: "description",
              label: "Description",
              render: (r) => <p className="font-medium">{r.description}</p>,
            },
            {
              key: "vendor",
              label: "Vendor",
              render: (r) => <span className="text-xs">{r.vendor ?? "—"}</span>,
            },
            {
              key: "payment_method",
              label: "Method",
              render: (r) => <span className="text-xs">{r.payment_method ?? "—"}</span>,
            },
            {
              key: "amount",
              label: "Amount",
              align: "right",
              render: (r) => (
                <span className="font-mono text-destructive">-${Number(r.amount).toFixed(2)}</span>
              ),
            },
          ]}
        />
