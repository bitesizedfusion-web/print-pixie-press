...
const PRODUCT_CONFIGS: Record<string, any> = {
  ...
  Postcards: {
    ...
    finishes: [
      "Gloss Lamination (Front)",
      "Matte Lamination (Front)",
      "Uncoated (Back for Writing)",
    ],
  },
  "Labels for Packaging": {
...
const CustomSelect = ({
  ...
  return (
    <div className="relative space-y-3">
      {label && (
        <label className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground ml-2">
          {label}
        </label>
      )}
              ...
              >
                ...
                <div className="p-4 bg-muted/10 border-t border-border">
                  <button
                    type="button"
                    onClick={() => setIsOpen(false)}
                    className="w-full h-12 rounded-2xl bg-muted hover:bg-muted/80 text-foreground font-medium transition-colors"
                  >
                    Cancel
                  </button>
                </div>
...
function GetQuotePage() {
  ...
  const generateAndSendOtp = async () => {
    ...
    if (success) {
      ...
      toast.error(
        "Failed to send verification email. Please check your network or email service configuration.",
      );
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!adminSuccess) {
      toast.error(
        "Form submitted but email notifications failed. Please check your email configuration.",
      );
    }
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 pt-32">
        <motion.div
          ...
          className="max-w-xl w-full bg-card border border-border rounded-3xl p-10 lg:p-16 text-center shadow-2xl relative overflow-hidden"
        >
          <div className="absolute top-0 left-0 w-full h-2 bg-foreground" />
          <div className="w-24 h-24 rounded-full bg-foreground/5 text-foreground flex items-center justify-center mx-auto mb-8 shadow-inner">
            <CheckCircle2 className="w-12 h-12" />
          </div>
          <h1 className="font-heading text-5xl mb-6">Quote Received!</h1>
          <p className="text-muted-foreground text-lg mb-12 leading-relaxed">
            Thank you. We have received your verified request and a confirmation has been sent to{" "}
            <span className="text-foreground font-semibold">{formData.email}</span>.
          </p>
          <div className="space-y-4">
            <Link
              to="/"
              className="flex items-center justify-center gap-3 w-full h-14 rounded-full bg-foreground text-background font-medium hover:bg-foreground/90 transition-all shadow-lg hover:shadow-xl"
            >
    ...
    return (
      <div className="min-h-[80vh] bg-background flex flex-col items-center justify-center pt-24 md:pt-40 p-6">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="max-w-md w-full bg-card border border-border rounded-3xl p-10 lg:p-12 text-center shadow-2xl border-t-4 border-t-foreground relative z-10"
        >
          <div className="w-16 h-16 rounded-2xl bg-foreground/5 text-foreground flex items-center justify-center mx-auto mb-6">
          ...
          <div className="space-y-6">
            <input
              ...
              className="w-full h-16 text-center text-3xl font-mono tracking-[0.5em] rounded-2xl border-2 border-border bg-muted/30 focus:border-foreground focus:ring-0 transition-all outline-none"
            />
            <button
              type="button"
              onClick={handleVerify}
              disabled={otp.length !== 4}
              className="w-full h-14 rounded-full bg-foreground text-background font-semibold flex items-center justify-center gap-2 disabled:opacity-50 transition-all shadow-lg"
            >
    ...
    <div className="min-h-screen bg-background pb-24">
      <section className="pt-32 pb-12 border-b border-border">
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 lg:px-12 text-center">
          <motion.div {...fadeUp}>
            <h1 className="font-heading text-[clamp(2.5rem,6vw,5rem)] font-light tracking-[-0.04em] leading-none text-foreground">
              Get a Quote
            </h1>
            <div className="mt-12 flex items-center justify-center gap-4 max-w-md mx-auto">
              {[1, 2, 3].map((s) => (
                <div key={s} className="flex-1 flex flex-col items-center gap-2">
                  <div
                    className={`w-full h-1.5 rounded-full transition-all duration-500 ${currentStep >= s ? "bg-foreground" : "bg-muted"}`}
                  />
                  <span
                    className={`text-[10px] font-mono uppercase tracking-widest ${currentStep === s ? "text-foreground font-bold" : "text-muted-foreground"}`}
                  >
                    Step 0{s}
                  </span>
                </div>
          ...
          <form onSubmit={handleSubmit} onKeyDown={handleKeyDown} className="max-w-4xl mx-auto">
            <AnimatePresence mode="wait">
              {currentStep === 1 && (
                ...
                >
                  <div className="space-y-8">
                    <h3 className="font-heading text-2xl border-b border-border pb-4">
                      Contact Details
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <input
                        ...
                        className="w-full h-14 px-5 rounded-2xl border border-border bg-card outline-none focus:border-foreground transition-all font-medium"
                      />
                      <input
                      ...
                      <div className="relative flex items-center">
                        ...
                        <input
                          ...
                          className="w-full h-14 pl-[84px] pr-5 rounded-2xl border border-border bg-card outline-none focus:border-foreground transition-all font-medium"
                        />
                      </div>
                    </div>
                  </div>
                  {isEditing && (
                    <div className="pt-8 border-t border-border flex justify-center">
                      <button
                        ...
                        className="px-8 h-14 rounded-full bg-foreground text-background font-medium hover:bg-foreground/90 transition-all flex items-center gap-2 shadow-lg"
                      >
              ...
              {currentStep === 2 && (
                ...
                >
                  <div className="space-y-8">
                    <h3 className="font-heading text-2xl border-b border-border pb-4 text-center">
                    ...
                    <div className="grid grid-cols-1 gap-6">
                      <CustomSelect
                        ...
                        value={
                          PRODUCT_CATEGORIES.some((g) => g.items.includes(formData.productName))
                      ...
                      {formData.productName && (
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-8 rounded-3xl bg-muted/20 border border-border"
                        >
                          ...
                          {(() => {
                            ...
                            const renderEditableSelect = (
                              ...
                              const isTrigger = CUSTOM_TRIGGERS.some((t) =>
                                      ...
                                      {[
                                        ...
                                        <div key={dim.id} className="space-y-2">
                                          <input
                                            ...
                                            onChange={(e) =>
                                              handleInputChange(dim.id, e.target.value)
                                            }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-12 pt-4">
                      <div className="space-y-4">
                        <div className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground text-center mb-2">
                          Delivery Details
                        </div>
                        ...
                        <div className="space-y-2">
                          {urgencyOptions.map((o) => (
                            <div key={o.id} className="space-y-2">
                              <button
                                type="button"
                                onClick={() => handleInputChange("urgency", o.id)}
                                className={`w-full text-left p-4 rounded-xl border transition-all ${formData.urgency === o.id ? "bg-foreground text-background border-foreground shadow-lg" : "bg-card border-border hover:border-foreground/30"}`}
                              >
                                <div className="text-sm font-semibold">{o.label}</div>
                                <div
                                  className={`text-[10px] mt-1 ${formData.urgency === o.id ? "text-background/70" : "text-muted-foreground"}`}
                                >
                                  {o.desc}
                                </div>
                              </button>
                    ...
                    <textarea
                      ...
                      className="w-full p-5 rounded-2xl border border-border bg-card outline-none focus:border-foreground transition-all resize-none font-medium"
                    />
                    {isEditing && (
                      <div className="pt-8 border-t border-border flex justify-center">
                        <button
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {imageSlots.map((s) => (
                        <label
                          ...
                          className="group relative aspect-square rounded-2xl border-2 border-dashed border-border bg-background hover:bg-muted/50 transition-all cursor-pointer overflow-hidden flex flex-col items-center justify-center p-4"
                        >
                          {previews[s.id] ? (
                            <img
                              src={previews[s.id]!}
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              e.stopPropagation();
                              handleImageChange(s.id, e.target.files?.[0] || null);
                            }}
                          />
                        </label>
                      ))}
                  ...
                  <div className="p-8 rounded-3xl bg-secondary/30 border border-border relative overflow-hidden group">
                    <div className="absolute top-4 right-6 flex gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground hover:text-foreground flex items-center gap-1.5 py-2 px-3 rounded-full bg-background/50 backdrop-blur-sm border border-border/50"
                      >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-4 text-sm">
                      <div className="flex justify-between border-b border-border/50 pb-3 group/row">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            ...
                            className="p-1 hover:bg-muted rounded text-muted-foreground hover:text-foreground md:opacity-0 group-hover/row:opacity-100 transition-opacity"
                          >
                      ...
                      {Object.entries(formData).map(([key, value]) => {
                        ...
                        return (
                          <div
                            key={key}
                            className="flex justify-between border-b border-border/50 pb-3 group/row"
                          >
            ...
            <div className="mt-16 flex items-center justify-center gap-6 pt-8 border-t border-border">
              {currentStep < 3 ? (
                <button
                  type="button"
                  onClick={nextStep}
                  className="max-w-md w-full h-16 rounded-full bg-foreground text-background font-semibold hover:bg-foreground/90 transition-all flex items-center justify-center gap-3 shadow-xl"
                >
                ...
                <button
                  type="submit"
                  className="max-w-md w-full h-16 rounded-full bg-gradient-brand text-white font-semibold text-lg shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all flex items-center justify-center gap-3"
                >
                  {status === "submitting" ? (
