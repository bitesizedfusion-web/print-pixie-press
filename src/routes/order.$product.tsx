...
export const Route = createFileRoute("/order/$product")({
  head: ({ params }) => {
    ...
    const title = product
      ? `${product.name} Printing — Instant Quote | PrintHub Australia`
      : "Configure Your Order — PrintHub Australia";
...
const faqs = [
  {
    ...
    a: "Configure your specifications using the Instant Quote Calculator on the right, click Add to Cart, then upload your artwork files in the next step. You can place orders without an account, but signing up earns rewards on every order.",
  },
  {
    q: "How do I design my files?",
    a: "You can use Adobe Photoshop, InDesign, Illustrator, Canva, or Affinity Publisher. Download our free PDF templates from the calculator to ensure your artwork is print-ready.",
  },
  {
    q: "What is bleed, and why do I need it?",
    a: "Bleed is a 3mm extension of artwork beyond the trim edge that prevents unprinted white edges when we cut your prints. Most items require 3mm bleed on every side.",
  },
  {
    q: "Which file types can I upload?",
    a: "PDF, AI, PSD, JPG, PNG, TIFF and more. We recommend print-ready PDFs at 300 DPI in CMYK colour mode for best results.",
  },
  {
    q: "When can I expect my order?",
    a: "Standard turnaround is 3–5 business days. Express turnaround is 1–2 business days for an additional 30%. Cut-off for confirmed orders is 4pm AEST every workday.",
  },
  {
    q: "What is lamination, and why do I need it?",
    a: "Lamination is a transparent plastic coating that protects, preserves and enhances your prints. It minimises marking, smudging, fading and tearing — choose Gloss, Matt or Soft-Touch.",
  },
];
    ...
    <div className="bg-background">
      ...
      <div className="border-b border-border bg-surface">
        <div className="max-w-7xl mx-auto px-4 py-3 text-xs text-muted-foreground flex items-center gap-2 overflow-x-auto whitespace-nowrap">
          ...
          <Link to="/products" className="hover:text-cta transition-colors">
            Products
          </Link>
          <ChevronRight className="h-3 w-3" />
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-7 space-y-8">
              {/* Title + Tabs */}
              <div>
                <p className="text-sm font-mono uppercase tracking-wider text-cta mb-2">
                  {product.category}
                </p>
                  ...
                  {tab === "info" ? (
                    ...
                    >
                      <p className="text-foreground/80 leading-relaxed">
                        Print custom {product.name.toLowerCase()} online with PrintHub Australia.
                        Our clever Instant Quote Calculator only displays the best options for your
                        prints — saving you time and money. Premium paper stocks, professional
                        finishes, and Australia-wide delivery on every order.
                      </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <div
                    key={t.label}
                    className="bg-card border border-border rounded-xl p-3 text-center hover:border-cta/40 transition-colors"
                  >
              ...
              <div className="lg:sticky lg:top-24">
                ...
                >
                  ...
                  <div className="bg-gradient-hero text-hero-foreground p-5 flex items-center justify-between">
                    <div>
                      <p className="text-[10px] font-mono uppercase tracking-widest text-hero-foreground/60">
                    ...
                    <Step n={2} label="Colour Printing">
                      <div className="grid grid-cols-2 gap-2">
                        {(
                          ...
                          <button
                            key={id}
                            onClick={() => setColourMode(id)}
                            className={chipCls(colourMode === id)}
                          >
                            {label}
                          </button>
                        ))}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {(
                          ...
                          <button
                            ...
                            className={chipCls(orientation === id)}
                          >
                            {label}
                          </button>
                        ))}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {(
                          ...
                          <button
                            ...
                            className={chipCls(doubleSided === ds)}
                          >
                            {label}
                            {ds ? <span className="text-[10px] opacity-70 ml-1">+20%</span> : null}
                          </button>
                        ))}
                      </div>
                    </Step>
                    ...
                    <Step n={8} label="Turnaround & Dispatch">
                      <div className="grid grid-cols-2 gap-2">
                        {(
                          ...
                          <button
                            key={id}
                            onClick={() => setTurnaround(id)}
                            className={chipCls(turnaround === id)}
                          >
                            {label}
                            {id === "express" ? (
                              <span className="text-[10px] opacity-70 ml-1">+30%</span>
                            ) : null}
                          </button>
                        ))}
                      </div>
                      <div className="mt-3 flex items-center gap-2 text-xs bg-success/10 text-success px-3 py-2 rounded-lg">
                        ...
                        <span>
                          Estimated dispatch: <strong>{dispatchDate}</strong>
                        </span>
                      </div>
                    </Step>
                          ...
                          {detectedFrame && (
                            <div className="flex items-center gap-2 text-[11px] bg-success/10 border border-success/30 text-success rounded-lg px-3 py-2">
                              ...
                              <span>
                                ...
                                {detectedAspect && ` (${detectedAspect.toFixed(2)}:1)`} —
                                orientation matched ✓
                              </span>
                            </div>
                          )}
                ...
                <div className="mt-4 bg-card border border-border rounded-2xl p-4 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Chat with a print expert · 1300 555 123
                    </p>
                  </div>
                  <Link to="/contact">
          ...
          <div className="mt-20">
            <div className="flex items-end justify-between mb-6">
              <div>
                <p className="text-xs font-mono uppercase tracking-wider text-cta">
                  More from PrintHub
                </p>
              ...
              <Link
                to="/products"
                className="text-sm font-medium text-cta hover:underline hidden sm:inline"
              >
                View all →
              </Link>
            </div>
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    We auto-detected your artwork as{" "}
                    <strong className="text-foreground">{detectedFrame}</strong>. Open the AR
                    preview to see exactly how it'll look framed in your room.
                  </p>
                </div>
              </div>
    ...
    <div className="space-y-2">
      ...
      >
        {added ? (
          ...
          <span className="flex items-center gap-2">
            <ShoppingCart className="h-4 w-4" /> Add to Cart · ${result.total.toFixed(2)}
          </span>
        )}
      </Button>
      {added && (
